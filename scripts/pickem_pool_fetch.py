...
from edge import pickem_log as _log  # noqa: E402
...
#: Kept as a module attribute so a test can redirect it (tests/conftest.py),
#: and so the page and the unit and this script all name the same file.
FAILURES_LOG = _log.FAILURES_LOG


def record_failure(reason: str) -> None:
    """Record a CBS fetch that did not happen, where the page will show it.

    Thin wrapper over edge.pickem_log.record_failure so there is one
    implementation of "append a line to the failures log", not two. See that
    function for why a non-zero exit is not enough on its own: this script
    runs as a soft-failing `ExecStartPre=-`, which discards the status.
    """
    _log.record_failure("cbs-pool-fetch", reason, path=FAILURES_LOG)
